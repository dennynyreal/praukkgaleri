...
if (!isset($_SESSION['user_id']) || $_SESSION['role'] != 'user') {
  header("Location: landing.php"); // Arahkan ke login jika bukan user biasa
  exit();
}
...
<body>
    ...
    <div class="container">
      ...
      <div class="collapse navbar-collapse justify-content-end" id="navbarNav">
        <ul class="navbar-nav">
          ...
          <li class="nav-item">
            <a class="nav-link <?php echo ($current_page == 'album.php') ? 'active' : ''; ?>" href="album.php">Album</a>
          </li>
          <li class="nav-item">
            <a class="nav-link <?php echo ($current_page == 'foto.php') ? 'active' : ''; ?>" href="foto.php">Foto</a>
          </li>
          <li class="nav-item">
  ...
  <div class="container my-5">
    <h2 class="text-start text-primary mb-4">Galeri Foto</h2><br>
    <div class="row">
      <?php
      if ($resultFoto->num_rows > 0) {
        while ($row = $resultFoto->fetch_assoc()) {
          ...
          echo '      <p class="card-text text-muted small">' . htmlspecialchars($row['deskripsifoto']) . '</p>';
          echo '      <p class="card-text"><small class="text-muted">Tanggal Unggah: ' . htmlspecialchars($row['tanggalunggah']) . '</small></p>';
          echo '      <div class="d-flex justify-content-between align-items-center">';
          echo '        <button class="btn btn-like" data-fotoid="' . $row['fotoid'] . '"><i class="fas fa-thumbs-up"></i> <span class="like-count">' . htmlspecialchars($row['like_count']) . '</span></button>';
          echo '        <button class="btn btn-outline-primary" data-bs-toggle="modal" data-bs-target="#komentarModal" data-fotoid="' . $row['fotoid'] . '">Komentar</button>';
          echo '      </div>';
          echo '    </div>';
          echo '  </div>';
          echo '</div>';
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <h6>Komentar:</h6>
          <div id="modalKomentarContainer"></div>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="modal fade" id="komentarModal" tabindex="-1" aria-labelledby="komentarModalLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="komentarModalLabel">Tambah Komentar</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body">
          <form action="aksi_komentar.php" method="POST">
            <input type="hidden" name="fotoid" id="fotoidKomentar">
            <div class="mb-3">
              <label class="form-label">Isi Komentar</label>
              <textarea class="form-control" name="isikomentar" required></textarea>
            </div>
      ...
      likeButtons.forEach(button => {
        button.addEventListener('click', function() {
          ...
          fetch('aksi_like.php', {
            ...
            .then(data => {
              if (data === "Like ditambahkan.") {
                likeCount += 1; // Tambahkan satu ke penghitung
                likeCountElement.innerText = likeCount; // Update penghitung
              }
              alert(data); // Tampilkan pesan
            })
            .catch(error => console.error('Error:', error));
        });
      });

      // Script untuk modal komentar
      const komentarButtons = document.querySelectorAll('.btn-outline-primary');
      komentarButtons.forEach(button => {
        button.addEventListener('click', function() {
          const fotoid = this.getAttribute('data-fotoid');
          document.getElementById('fotoidKomentar').value = fotoid;
        });
      });
    });
  </script>
